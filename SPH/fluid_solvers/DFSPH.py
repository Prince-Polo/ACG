...
class DFSPHSolver(BaseSolver):
    ...
    def compute_alpha_task(self, p_i, p_j, ret: ti.template()):
        ...
        if self.container.particle_materials[p_j] == self.container.material_fluid:
            # Fluid neighbors
            grad_p_j = -self.container.particle_rest_volumes[p_j] * self.kernel.gradient(r = self.container.particle_positions[p_i] - self.container.particle_positions[p_j], h = self.container.dh)
    ...
    def compute_density_derivative_task(self, p_i, p_j, ret: ti.template()):
        ...
        ret.density_adv += self.container.particle_rest_volumes[p_j] * ti.math.dot(v_i - v_j, self.kernel.gradient(pos_i - pos_j, self.container.dh))
    ...
    def compute_density_star_task(self, p_i, p_j, ret: ti.template()):
        ...
        ret += self.container.particle_rest_volumes[p_j] * ti.math.dot(v_i - v_j,self.kernel.gradient(pos_i - pos_j, self.container.dh))
    ...
    def correct_divergence_task(self, p_i, p_j, ret: ti.template()):
        if self.container.particle_materials[p_j] == self.container.material_fluid:
            ...
            if ti.abs(k_sum) > self.m_eps * self.dt[None]:
                grad_p_j = self.container.particle_rest_volumes[p_j] * self.kernel.gradient(self.container.particle_positions[p_i] - self.container.particle_positions[p_j], self.container.dh)
