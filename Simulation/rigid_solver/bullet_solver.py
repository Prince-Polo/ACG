...
class PyBulletSolver():
    ...
    def init_rigid_body(self, rigid_body):
        ...
        self.container_idx_to_bullet_idx[container_idx] = container_idx
        self.bullet_idx_to_container_idx[container_idx] = container_idx
    ...
    def apply_force(self, container_idx, force: Tuple[float, float, float]):
        ...
        com_pos = self.container.rigid_body_centers_of_mass[bullet_idx]
        self.container.rigid_body_forces[bullet_idx] += np.array(force)

    def apply_torque(self, container_idx, torque: Tuple[float, float, float]):
        bullet_idx = self.container_idx_to_bullet_idx[container_idx]
        self.container.rigid_body_torques[bullet_idx] += np.array(torque)
    ...
    def get_rigid_body_states(self, container_idx):
        bullet_idx = self.container_idx_to_bullet_idx[container_idx]
        linear_velocity = self.container.rigid_body_velocities[bullet_idx]
        angular_velocity = self.container.rigid_body_angular_velocities[bullet_idx]
        position = self.container.rigid_body_centers_of_mass[bullet_idx]
        rotation_matrix = self.container.rigid_body_rotations[bullet_idx]
